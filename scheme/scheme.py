...
def eval_all(expressions, env):
    ...
    """
    # BEGIN PROBLEM 7
    # following your feeling, Not bad. Now you could see how the function works below.
    if expressions == nil:
        return None
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
        return self.env.make_child_frame(self.formals, args)  # 对自己的子框架进行参数值绑定
        # END PROBLEM 11
...
def do_and_form(expressions, env):
    ...
    while isinstance(expressions, Pair):
        ...
        if not is_true_primitive(eval_result):
            return eval_result
...
def do_or_form(expressions, env):
    ...
    while isinstance(expressions, Pair):
        eval_result = scheme_eval(expressions.first, env)
        if is_true_primitive(eval_result):
            return eval_result
        expressions = expressions.rest
    if expressions is not nil:
        raise TypeError
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            if clause.rest is nil:  # if eval_all as cond, then the x is computed twice.
                return test
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    while isinstance(bindings, Pair):  # sikaolehaoyihuier,meiyoudaan,burucankaobierendedaan
...
class MuProcedure(Procedure):
    ...
    # BEGIN PROBLEM 15
    # 在碰到'mu'后。对应的特殊形式，把后面的参数，表达式生成一个MuPeocedure实例, 在计算的时候，引用make_call_frame
    "*** YOUR CODE HERE ***"
    def make_call_frame(self, args, env):
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 18
    "*** YOUR CODE HERE ***"
    return MuProcedure(formals, expressions.rest)  # refer to LambdaProcedure
    # END PROBLEM 18
